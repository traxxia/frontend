...
const ProjectReviewModal = ({
  ...
  return <div className="review-modal-overlay">
            <div className="review-modal-content">
                ...
                <form onSubmit={handleSubmit} className="review-modal-body">
                    {popupError && <div className="error-popup-overlay">
                    ...
                    {showConfirmation ? <div className="confirmation-view project-review-modal--s1">
                            <AlertTriangle size={48} color="#d97706" className="project-review-modal--s2" />
                            <h3 className="project-review-modal--s3">{t("Confirm_Submission") || "Confirm Submission"}</h3>
                            <p className="project-review-modal--s4">
                                {type === "review" ? t("Confirm_Review_Message") || "You are about to log this Strategic Review. This action will be permanently recorded in the project's Decision Log. Are you sure you want to proceed?" : t("Confirm_AdHoc_Message") || "You are about to log an Ad-Hoc Update. This will be recorded in the project's Decision Log. Are you sure you want to proceed?"}
                            </p>

                            <div className="review-modal-footer project-review-modal--s5">
                  ...
                  color: project.is_stale ? "#ef4444" : "inherit",
                  fontWeight: project.is_stale ? "700" : "600"
                }}>
                            ...
                            {type === "review" && <div className="form-group checkbox-group">
                                    <label className="checkbox-container">
                                        <input type="checkbox" checked={noChanges} onChange={e => setNoChanges(e.target.checked)} />
                            ...
                            <div className={`form-section ${noChanges ? 'disabled' : ''}`}>
                                <div className="grid-2">
                                    <div className="form-group">
                                        ...
                                        <select value={status} onChange={e => setStatus(e.target.value)} disabled={noChanges}>
                                            {statusOptions.map(opt => <option key={opt} value={opt}>{t(opt)}</option>)}
                                        </select>
                                    </div>
                                    <div className="form-group">
                                        <label>{t("Learning_State")}</label>
                                        <select value={learningState} onChange={e => setLearningState(e.target.value)} disabled={noChanges}>
                                            {learningOptions.map(opt => <option key={opt} value={opt}>{t(opt)}</option>)}
                                        </select>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="form-group">
                                <label>
                                    <MessageSquare size={14} className="project-review-modal--s6" />
                                    {t("Justification")} *
                                </label>
                                <textarea value={justification} onChange={e => {
              setJustification(e.target.value);
              if (popupError) setPopupError("");
            }} placeholder={t("Justification_placeholder") || "Why is this change/review being made?"} required rows={4} />
                            </div>
