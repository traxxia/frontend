...
const CapabilityHeatmap = ({
  ...
  const handleMissingQuestionsCheck = async () => {
    ...
    await checkMissingQuestionsAndRedirect('CapabilityHeatmap', selectedBusinessId, handleRedirectToBrief, {
  ...
  const containsNotEnoughData = data => {
    ...
    if (data.capabilities && Array.isArray(data.capabilities)) {
      for (const capability of data.capabilities) {
        if (hasNotEnoughDataValue(capability.name) || hasNotEnoughDataValue(capability.category) || hasNotEnoughDataValue(capability.type) || hasNotEnoughDataValue(capability.impact) || hasNotEnoughDataValue(capability.currentLevel)) {
          return true;
        }
      }
    }
  ...
  const isCapabilityDataIncomplete = data => {
    ...
    const validCapabilities = data.capabilities.filter(capability => capability.name && !hasNotEnoughDataValue(capability.name) && capability.category && !hasNotEnoughDataValue(capability.category) && capability.type && !hasNotEnoughDataValue(capability.type) && capability.currentLevel !== null && capability.currentLevel !== undefined && !hasNotEnoughDataValue(capability.currentLevel));
  ...
  const getCellColor = (capability, maturityLevel) => {
    if (capability.currentLevel === maturityLevel) {
      return capability.type === 'strength' ? capability.impact === 'high' ? '#10b981' : '#34d399' : capability.impact === 'high' ? '#ef4444' : '#f87171';
  ...
  if (isRegenerating) {
    return <div className="capability-heatmap">
  ...
  if (error || !capabilityData || isCapabilityDataIncomplete(capabilityData)) {
    return <div className="capability-heatmap">
        <AnalysisEmptyState analysisType="capabilityHeatmap" analysisDisplayName="Capability Heatmap Analysis" icon={Zap} onImproveAnswers={handleMissingQuestionsCheck} onRegenerate={handleRegenerate} isRegenerating={isRegenerating} canRegenerate={canRegenerate} userAnswers={userAnswers} minimumAnswersRequired={3} showImproveButton={false} showRegenerateButton={false} />
      </div>;
  }
  return <div className="capability-heatmap" data-analysis-type="capability-heatmap" data-analysis-name="Capability Heatmap" data-analysis-order="5">
          ...
          <div className="ch-heatmap-wrapper">
            <div className="ch-heatmap">
              <div className="ch-heatmap-header">
                ...
                {maturityLevels.map(level => <div key={level.level} className="ch-cell ch-cell-header ch-maturity-header">
                    <div className="ch-maturity-level">Level {level.level}</div>
                    <div className="ch-maturity-label">{level.label}</div>
                  </div>)}
              </div>

              {validCapabilities.map(capability => <div key={capability.name} className="ch-heatmap-row">
                ...
                return <div key={cellKey} className={`ch-cell ch-cell-data ${selectedCell === cellKey ? 'selected' : ''} ${isCurrentLevel ? 'active-level' : ''} capability-heatmap--s1`} style={{
                  backgroundColor: getCellColor(capability, level.level)
                }} onClick={() => setSelectedCell(cellKey === selectedCell ? null : cellKey)} onMouseEnter={() => setHoveredCell(cellKey)} onMouseLeave={() => setHoveredCell(null)}>
                        <div className="ch-cell-content">
                          {isCurrentLevel && <div className="ch-level-indicator">
                              <div className="ch-level-dot"></div>
                              Current
                            </div>}
                        </div>
                        {hoveredCell === cellKey && <div className="ch-tooltip">
                            ...
                            <div className="ch-tooltip-content">
                              ...
                              {capability.targetLevel && <div>Target Level: {capability.targetLevel}</div>}
                            </div>
                          </div>}
                      </div>;
              })}
