...
import { Award, BarChart3, Users, Cog, Star, Zap, Loader, ChevronDown, ChevronRight } from 'lucide-react';
...
const MaturityScore = ({
  ...
  const handleMissingQuestionsCheck = useCallback(async () => {
    ...
    await checkMissingQuestionsAndRedirect('maturityScore', selectedBusinessId, handleRedirectToBrief, {
    ...
    if (scoreData.dimensions?.length) {
      scoreData.dimensions.forEach(dimension => {
        ...
        dimension.subDimensions?.forEach(subDim => {
          if (subDim.score >= 4.0) transformed.strengths.push(`${subDim.name}: ${subDim.description}`);else if (subDim.score < 3.7) transformed.developmentAreas.push(`${subDim.name}: ${subDim.description}`);
        });
      });
    }
      ...
      if (currentRow < totalItems) {
        ...
        if (currentTransformed.nextLevel) {
          ...
          if (progIndex < progRows) {
            if (progIndex === 0) typeText(currentTransformed.nextLevel.target, currentRow, 'target', 0);else if (progIndex === 1) typeText(currentTransformed.nextLevel.estimatedTimeframe, currentRow, 'timeline', 0);else if (progIndex === 2 && currentTransformed.nextLevel.investment) typeText(currentTransformed.nextLevel.investment, currentRow, 'investment', 0);
            currentRow++;
            return;
          }
  ...
  if (isRegenerating) {
    return <div className="maturity-container">
  ...
  if (!data || isMaturityDataIncomplete(data)) {
    return <div className="maturity-score-container">
        <AnalysisEmptyState analysisType="maturityScore" analysisDisplayName="Maturity Score Analysis" icon={Award} onImproveAnswers={handleMissingQuestionsCheck} onRegenerate={handleRegenerate} isRegenerating={isRegenerating} canRegenerate={canRegenerate} userAnswers={userAnswers} minimumAnswersRequired={3} showImproveButton={false} showRegenerateButton={false} />
      </div>;
  }
  const renderGaugeChart = transformedData => <div className="gauge-section">
      <div className="gauge-container">
        ...
        <div className="gauge-content">
          ...
          color: getLevelColor(transformedData.level)
        }}>{transformedData.level}</div>
    ...
    return <div className="section-container">
        ...
        <div className={`section-container ${expandedSections.areas === true ? 'expanded' : 'collapsed'}`}>
          <div className="table-container pt-3">
            ...
            {transformedData.dimensions.some(d => d.subDimensions?.length) && <div className="subsection border-0 pt-0 mt-0">
                <table className="data-table">
                  <thead>
                    <tr><th>{t('Area')}</th><th>{t('Sub-Dimension')}</th><th>{t('Score')}</th><th>{t('description')}</th></tr>
                  </thead>
                  <tbody>
                    {transformedData.dimensions.map(dimension => dimension.subDimensions?.map((subDim, subIndex) => {
                  const rowIndex = subDimensionIndices[`${dimension.name}-${subIndex}`];
                  const isVisible = rowIndex < visibleRows;
                  return <StreamingRow key={`${dimension.name}-${subIndex}`} isVisible={isVisible} isLast={rowIndex === visibleRows - 1 && isStreaming} lastRowRef={lastRowRef} isStreaming={isStreaming}>
  ...
  const renderCrossScoring = transformedData => {
    ...
    if (transformedData.dimensions?.length) {
      ...
      transformedData.dimensions.forEach(dim => {
        if (dim.subDimensions?.length) currentRowIndex += dim.subDimensions.length;
      });
    }
    ...
    return <div className="section-container">
        ...
        <div className={`section-container ${expandedSections.crossScoring === true ? 'expanded' : 'collapsed'}`}>
          <div className="table-container">
            {correlations.length > 0 && <div className="subsection">
                ...
                <table className="data-table">
                  ...
                  const isVisible = rowIndex < visibleRows;
                  return <StreamingRow key={idx} isVisible={isVisible} isLast={rowIndex === visibleRows - 1 && isStreaming} lastRowRef={lastRowRef} isStreaming={isStreaming}>
            ...
            {synergies.length > 0 && <div className="subsection">
          ...
          <div className="table-container">
            <table className="data-table">
              ...
              <tbody>
                ...
                {nextLevel.investment && <StreamingRow isVisible={currentRowIndex + 2 < visibleRows} isLast={currentRowIndex + 2 === visibleRows - 1 && isStreaming} lastRowRef={lastRowRef} isStreaming={isStreaming}>
            ...
            {nextLevel.requirements?.length > 0 && <div className="subsection">
                ...
                <table className="data-table">
                  ...
                  return <StreamingRow key={reqIdx} isVisible={isVisible} isLast={reqRowIndex === visibleRows - 1 && isStreaming} lastRowRef={lastRowRef} isStreaming={isStreaming}>
                          <td className="maturity-score-light--s1"><strong>{reqIdx + 1}</strong></td>
  ...
  return <div className="maturity-container" data-analysis-type="maturityScore" data-analysis-name="Business Maturity Score" data-analysis-order="8">
