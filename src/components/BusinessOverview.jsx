...
const BusinessOverview = ({
  ...
  const handleSearch = value => {
    if (value && !searchTerm) setPageBeforeSearch(currentPage);else if (!value && searchTerm) setCurrentPage(pageBeforeSearch);
    setSearchTerm(value);
  };
  const filteredBusinesses = React.useMemo(() => {
    ...
    return businesses.filter(biz => {
      const collaboratorText = (biz.collaborators || []).map(c => `${c.name} ${c.email}`).join(" ").toLowerCase();
      return biz.business_name?.toLowerCase().includes(search) || biz.owner_name?.toLowerCase().includes(search) || biz.owner_email?.toLowerCase().includes(search) || collaboratorText.includes(search) || new Date(biz.created_at).toLocaleDateString().toLowerCase().includes(search);
    });
  ...
  const columns = [{
    ...
    render: (_, row) => {
      ...
      if (collabs.length === 0) {
        return <span className="admin-cell-secondary">
                            <Users size={13} className="business-overview--s1" />
                            {t("no_collaborators")}
                        </span>;
      }
      return <div className="business-overview--s2">
                        {displayed.map((c, idx) => <div key={idx} className="admin-collab-inline-item">
                                <div className="admin-collab-info">
                                    <div className="admin-cell-primary business-overview--s3">{c.name}</div>
                                    <div className="admin-cell-secondary">{c.email}</div>
                                </div>
                                {c.role_name !== "super_admin" && c.role_name !== "company_admin" && <button className="admin-collab-remove-btn inline" onClick={() => handleRemoveParticipant(row._id, c.id, c.name)} title={t("remove_participant") || "Remove Participant"}>
                                        <Trash2 size={12} />
                                    </button>}
                            </div>)}
                        {hasMore && <button className="admin-collab-more-btn" onClick={() => handleShowAllCollaborators(row)}>
    ...
    render: (_, row) => {
      ...
      }} className="business-overview--s4">
                        {label}
                    </span>;
    }
  }, {
  ...
  return <div>
            ...
            {}
            <AdminTable title={t("business_overview") || "Business Overview"} count={filteredBusinesses.length} countLabel={filteredBusinesses.length === 1 ? t("business") || "Business" : t("businesses") || "Businesses"} columns={columns} data={paginatedBusinesses} searchTerm={searchTerm} onSearchChange={handleSearch} searchPlaceholder={t("search_by_business_owner") || "Search by business or owner..."} searchTooltip={t("search_businesses_tooltip")} currentPage={currentPage} totalPages={totalPages} onPageChange={setCurrentPage} totalItems={totalItems} itemsPerPage={itemsPerPage} emptyMessage={t("no_businesses_found") || "No businesses found"} loading={loading} />

            {}
            {showCollabModal && selectedBizForCollab && <div className="admin-modal-overlay" onClick={() => setShowCollabModal(false)}>
                    <div className="admin-modal-box" onClick={e => e.stopPropagation()}>
                        <div className="admin-modal-header">
                            <h3>
                                <Users size={18} className="business-overview--s5" />
                                {selectedBizForCollab.business_name} — {t("participants")}
                            </h3>
                            <button className="admin-modal-close" onClick={() => setShowCollabModal(false)}>
                                <X size={18} />
                            </button>
                        </div>
                        <div className="admin-modal-body">
                            {selectedBizForCollab.collaborators?.map((collab, idx) => <div key={idx} className="admin-collab-item">
                                    ...
                                    {collab.role_name !== "super_admin" && collab.role_name !== "company_admin" && <button className="admin-collab-remove-btn" onClick={() => handleRemoveParticipant(selectedBizForCollab._id, collab.id, collab.name)} title={t("remove_participant") || "Remove Participant"}>
                                            <Trash2 size={16} />
                                        </button>}
                                </div>)}
                        </div>
                        <div className="admin-modal-footer">
                            <button className="admin-modal-footer-btn" onClick={() => setShowCollabModal(false)}>
                                {t("close") || "Close"}
                            </button>
                        </div>
                    </div>
            ...
            {}
            {showConfirmModal && <div className="admin-modal-overlay" onClick={() => setShowConfirmModal(false)}>
                    <div className="admin-modal-box confirm-box business-overview--s6" onClick={e => e.stopPropagation()}>
                        <div className="admin-modal-header">
                        ...
                        <div className="admin-modal-body">
                            <p className="mb-2 business-overview--s7">
                            ...
                            <p className="business-overview--s8">
                                <AlertCircle size={14} className="business-overview--s9" />
                            ...
                            <p className="mt-2 business-overview--s8">
                                <AlertCircle size={14} className="business-overview--s9" />
                        ...
                        <div className="admin-modal-footer business-overview--s10">
                            <button className="admin-modal-footer-btn" onClick={() => setShowConfirmModal(false)}>
                                {t("cancel") || "Cancel"}
                            </button>
                            <button className="admin-primary-btn business-overview--s11" onClick={confirmRemoveParticipant}>
