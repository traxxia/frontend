...
const StrategicGoals = ({
  ...
  const handleMissingQuestionsCheck = useCallback(async () => {
    ...
    await checkMissingQuestionsAndRedirect("strategicGoals", selectedBusinessId, handleRedirectToBrief, {
  ...
  const formatTheme = theme => {
    switch (theme?.toLowerCase()) {
      ...
      case "efficiency":
        return "Efficiency";
      default:
        return theme?.replace(/_/g, " ").replace(/\b\w/g, l => l.toUpperCase()) || theme;
    }
  };
          ...
          <div className="table-container">
            <div className="gantt-chart">
              <div className="timeline-header">
                ...
                {months.map((month, index) => <div key={index} className="month-label">
                    {month}
                  </div>)}
              </div>

              {objectivesWithTimeline.map((objective, index) => <div key={index} className="initiative-row">
                  <div className="initiative-name">{objective.objective}</div>
                  {months.map((_, monthIndex) => {
                const isActive = monthIndex >= objective.startMonth - 1 && monthIndex < objective.startMonth - 1 + objective.duration;
                ...
                const avgProgress = objective.keyResults?.length > 0 ? Math.round(objective.keyResults.reduce((sum, kr) => sum + (kr.progress || 0), 0) / objective.keyResults.length) : 0;
                return <div key={monthIndex} className="timeline-cell">
                        {isActive && <div className={`timeline-bar priority-${objective.priority} strategic-goals--s1`}>
                            {isMidPoint && objective.duration > 2 && <span className="strategic-goals--s2">
                                {avgProgress}%
                              </span>}
                            {(isFirstMonth && objective.duration <= 2 || isFirstMonth && !isMidPoint) && <span className="strategic-goals--s3">
    ...
    return <div className="section-container">
        ...
        <div className={`section-container ${expandedSections.overview === true ? 'expanded' : 'collapsed'}`}>
          <div className="table-container">
            <table className="data-table">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    {data.objectives?.filter(obj => obj.priority === 1).length || 0}
                  </td>
                  <td>-</td>
                </tr>
              </tbody>
            ...
            {themes && themes.length > 0 && <div className="subsection">
                <h4>Strategic Themes</h4>
                <div className="forces-tags">
                  {themes.map((theme, index) => <span key={index} className="force-tag">
                      {formatTheme(theme)}
                    </span>)}
                </div>
              </div>}
                ...
                return <tr key={index}>
                      ...
                      <td>
                        <div className="factors-cell">
                          {objective.keyResults?.map((kr, krIndex) => <div key={krIndex} className="factor-item">
  ...
  const renderKeyResultsDetailTable = objectives => {
    const allKeyResults = objectives.flatMap((objective, objIndex) => (objective.keyResults || []).map((kr, krIndex) => ({
      ...kr,
        ...
        <div className={`section-container ${expandedSections.keyresults === true ? 'expanded' : 'collapsed'}`}>
          <div className="table-container">
            <table className="data-table">
              ...
              <tbody>
                {allKeyResults.map((kr, index) => <tr key={index}>
                    <td>
  ...
  if (isRegenerating) {
    return <div className="strategic-goals-container">
  ...
  const renderContent = () => {
    if (error) {
      return <AnalysisError error={error} onRetry={handleRegenerate} title="Strategic Goals Analysis Error" />;
    }
    if (!data || isStrategicGoalsDataIncomplete(data)) {
      return <AnalysisEmptyState analysisType="strategicGoals" analysisDisplayName="Strategic Goals & OKR Analysis" icon={Target} onImproveAnswers={handleMissingQuestionsCheck} onRegenerate={handleRegenerate} isRegenerating={isRegenerating} canRegenerate={canRegenerate} userAnswers={userAnswers} minimumAnswersRequired={3} />;
    }
    return <div className="goals-content">
