...
const QuestionManagement = ({
  ...
  return <div className="question-management">
      <div className="admin-metrics-grid question-management--s1">
        <MetricCard label={t("total_questions") || "Total Questions"} value={questions.length} icon={FileText} iconColor="blue" />
        <MetricCard label={t("initial_phase") || "Initial Phase"} value={questionsByPhase.initial?.length || 0} icon={CheckCircle2} iconColor="green" />
        <MetricCard label={t("essential_phase") || "Essential Phase"} value={questionsByPhase.essential?.length || 0} icon={ListChecks} iconColor="orange" />
        <MetricCard label={t("advanced_phase") || "Advanced Phase"} value={questionsByPhase.advanced?.length || 0} icon={Layers} iconColor="purple" />
      </div>

      <div className="question-management__header">
        <h2>{t('question_management')}</h2>
        <button onClick={() => setShowCreateForm(true)} className="admin-primary-btn">
          <Plus size={16} />
          {t('add_question')}
        </button>
      </div>
      ...
      {}
      {isLoading && <div className="admin-loading-bar-container question-management--s2">
          <div className="admin-loading-bar" />
        </div>}

      {showCreateForm && <CreateQuestionForm onSubmit={handleCreateQuestion} onCancel={() => setShowCreateForm(false)} isLoading={isCreating} />}

      {editingQuestion && <EditQuestionModal question={editingQuestion} onUpdate={handleUpdateQuestion} onCancel={() => setEditingQuestion(null)} isUpdating={isUpdating} />}

      {showDeleteModal && <DeleteQuestionModal onConfirm={handleDeleteQuestion} onCancel={() => {
      setShowDeleteModal(false);
      setQuestionToDelete(null);
    }} isDeleting={!!deletingQuestionId} />}

      {isReordering && <div className="question-management__reordering-overlay">
          <div className="question-management__reordering-modal">
            <div className="admin-spinner question-management--s3" />
      ...
      const columns = [{
        ...
        label: '#',
        width: '80px',
        render: (_, row, idx) => <div className="question-management--s4">
                <span className="admin-cell-secondary">{idx + 1}</span>
              </div>
      }, {
        key: 'question_text',
        label: t('question_text'),
        render: (val, row) => <div className="question-text-cell">
                <div className="admin-cell-primary">{val}</div>
                {(row.objective || row.required_info) && <div className="admin-cell-secondary question-management--s5">
                    {row.objective}
                  </div>}
              </div>
      }, {
        ...
        key: 'severity',
        label: t('severity'),
        render: val => <span className={`admin-status-badge ${val === 'mandatory' ? 'active' : 'inactive'}`}>
                {val === "mandatory" ? t("mandatory") : val === "optional" ? t("optional") : val}
              </span>
      }, {
        key: 'actions',
        label: t('actions'),
        render: (_, row) => <div className="question-row__actions question-management--s6">
                <button onClick={() => setEditingQuestion(row)} className="question-row__action-btn question-row__action-btn--edit">
                  <Edit size={14} />
                </button>
                <button onClick={() => {
      ...
      return <div key={phase} className="phase-section-wrapper question-management--s1">
            <div onClick={() => togglePhaseCollapse(phase)} className="phase-section__header question-management--s7">
              {isCollapsed ? <ChevronRight size={20} /> : <ChevronDown size={20} />}
              {Icon && <Icon size={20} style={{
            color: iconColor
          }} />}
              <strong className="question-management--s8">
            ...
            {!isCollapsed && <AdminTable columns={columns} data={phaseQuestions} loading={isLoading} getRowProps={row => ({
          draggable: true,
      ...
      {questions.length === 0 && <div className="question-management__empty-state">
          <HelpCircle size={48} />
...
const EditQuestionModal = ({
  ...
  return <div className="create-form-overlay">
      <div className="create-form">
        <div className="question-management--s9">
          <h3 className="question-management--s10">{t('edit_question')}</h3>
          <button onClick={onCancel} className="admin-modal-close question-management--s11">
            <X size={20} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="create-form__field">
            ...
            <textarea name="question_text" value={formData.question_text} onChange={handleChange} required rows="4" />
          ...
          <div className="create-form__grid">
            ...
            <div className="create-form__field">
              ...
              <input type="text" name="used_for" value={formData.used_for} onChange={handleChange} required placeholder="SWOT, Customer Segmentation..." />
          ...
          <div className="create-form__field">
            ...
            <textarea name="objective" value={formData.objective} onChange={handleChange} required rows="3" />
            {errors.objective && <span className="error-message">{errors.objective}</span>}
          </div>

          <div className="create-form__field">
            <label>{t('required_information')}</label>
            <textarea name="required_info" value={formData.required_info} onChange={handleChange} rows="3" />
          </div>
          ...
          <div className="create-form__severity-info">
            ...
            <div className="create-form__severity-display">
              ...
              <span className="create-form__severity-text">
                {formData.phase === 'initial' ? t('initial_questions_mandatory') : t('non_initial_optional')}
              </span>
            </div>
          </div>
...
const CreateQuestionForm = ({
  ...
  return <div className="create-form-overlay">
      <div className="create-form">
        <div className="question-management--s9">
          <h3 className="question-management--s10">{t('create_new_question')}</h3>
          <button onClick={onCancel} className="admin-modal-close question-management--s11">
            <X size={20} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="create-form__field">
            <label>{t('question_text')} *</label>
            <textarea name="question_text" value={formData.question_text} onChange={handleChange} required rows="4" placeholder={t('enter_your_question_here')} />
            {errors.question_text && <span className="error-message">{errors.question_text}</span>}
          </div>
          ...
          <div className="create-form__field">
            <label>{t('objective')} *</label>
            <textarea name="objective" value={formData.objective} onChange={handleChange} required rows="3" placeholder={t('what_this_question_aims_to_understand')} />
            {errors.objective && <span className="error-message">{errors.objective}</span>}
          </div>

          <div className="create-form__field">
            <label>{t('required_information')}</label>
            <textarea name="required_info" value={formData.required_info} onChange={handleChange} rows="3" placeholder={t('what_specific_information_is_needed_for_analysis')} />
          </div>
...
const DeleteQuestionModal = ({
  ...
  return <div className="create-form-overlay">
      <div className="create-form question-management--s12">
        <div className="question-management--s9">
          <h3 className="question-management--s13">
            <AlertCircle size={20} />
            {t('Delete Question')}
          </h3>
          <button onClick={onCancel} className="admin-modal-close question-management--s11">
        ...
        <div className="question-management--s1">
          <p className="question-management--s14">
            Are you sure you want to delete this question?
          </p>
          <div className="question-management--s15">
        ...
        <div className="create-form__actions">
          ...
          <button onClick={onConfirm} disabled={isDeleting} className="create-form__btn create-form__btn--submit question-management--s16">
