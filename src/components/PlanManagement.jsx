...
const PlanModal = ({
  ...
  useEffect(() => {
    if (plan) {
      setFormData({
        ...
        workspace_limit: plan.limits?.workspaces !== undefined ? plan.limits.workspaces : plan.workspace_limit !== undefined ? plan.workspace_limit : plan.max_workspaces !== undefined ? plan.max_workspaces : '',
        limit_projects: plan.limits?.project !== undefined ? Boolean(plan.limits.project) : plan.limits?.projects !== undefined ? Boolean(plan.limits.projects) : plan.can_create_projects !== undefined ? Boolean(plan.can_create_projects) : plan.max_projects !== undefined ? Boolean(plan.max_projects) : false,
        limit_collaborators: plan.limits?.collaborators !== undefined ? plan.limits.collaborators : plan.max_collaborators !== undefined ? plan.max_collaborators : '',
        limit_viewers: plan.limits?.viewers !== undefined ? plan.limits.viewers : plan.max_viewers !== undefined ? plan.max_viewers : '',
        limit_users: plan.limits?.users !== undefined ? plan.limits.users : plan.max_users !== undefined ? plan.max_users : '',
        insight: plan.limits?.insight !== undefined ? plan.limits.insight : plan.insight !== undefined ? plan.insight : false,
        strategic: plan.limits?.strategic !== undefined ? plan.limits.strategic : plan.strategic !== undefined ? plan.strategic : false,
        pmf: plan.limits?.pmf !== undefined ? plan.limits.pmf : plan.pmf !== undefined ? plan.pmf : false,
  ...
  }, [formData.workspace_limit, formData.limit_users, formData.limit_collaborators, formData.limit_viewers, formData.insight, formData.strategic, formData.pmf, formData.limit_projects]);
  if (!show) return null;
  ...
  const handleSubmit = e => {
    ...
    if (workspaceLimitValue !== undefined && (isNaN(workspaceLimitValue) || workspaceLimitValue < 0) || maxCollaborators !== undefined && (isNaN(maxCollaborators) || maxCollaborators < 0) || maxViewers !== undefined && (isNaN(maxViewers) || maxViewers < 0) || maxUsers !== undefined && (isNaN(maxUsers) || maxUsers < 0)) {
...
const PlanManagement = ({
  ...
  const handleSavePlan = async planData => {
    ...
    try {
      ...
      const url = isEdit ? `${API_BASE_URL}/api/plans/${modalState.plan._id}` : `${API_BASE_URL}/api/plans`;
