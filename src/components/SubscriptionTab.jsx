...
import { Check, CreditCard, Plus, Trash2, Star, TrendingUp, Layers, Target, Zap, Users, Briefcase, Eye, FolderOpen, ChevronRight, Lock, AlertCircle, X, Trash } from "lucide-react";
...
const buildFeatureRows = (limits = {}) => [{
  ...
  label: limits.viewers ? `${limits.viewers} Viewer${limits.viewers !== 1 ? "s" : ""}` : "Viewers",
  active: !!limits.viewers
}, {
  label: limits.users ? `${limits.users} User${limits.users !== 1 ? "s" : ""}` : "Users",
  active: !!limits.users
}, {
  label: "Projects",
  active: !!limits.project
}, {
  label: "Insight Access",
  active: !!limits.insight
}, {
  label: "Strategic Access",
  active: !!limits.strategic
}, {
  label: "PMF Access",
  active: !!limits.pmf
}];
const UsageBar = ({
  ...
  return <div className="st-u-row">
      ...
      <div className="st-u-track">
        <div className={`st-u-fill st-fill-${color}`} style={{
        width: `${pct}%`,
        ...(isHigh ? {
          background: "#ef4444"
        } : {})
...
const FeatureRow = ({
  ...
  active
    ...
    <span className={`st-feat-check ${active ? "st-feat-check-on" : "st-feat-check-off"}`}>
...
const AddCardModalContent = ({
  ...
  const handleLocalSubmit = async () => {
    ...
    try {
      ...
      const isDuplicate = paymentMethods.some(pm => pm.last4 === card.last4 && pm.brand === card.brand && pm.exp_month === card.exp_month && pm.exp_year === card.exp_year);
      if (isDuplicate) {
        setError(t("This card is already linked to your account."));
        setIsSubmitting(false);
        return;
      }
      const res = await fetch(`${apiBase}/api/subscription/payment-methods/add`, {
        method: "POST",
        headers: {
      ...
      if (onToast) onToast(t("card_added_success") || "Card added successfully", "success");
      onAddSuccess(data);
      onHide();
    } catch (err) {
  ...
  return <div className="add-card-modal-inner">
      <Modal.Header closeButton className="border-0 pb-0">
        <Modal.Title className="fw-bold">{t("Add New Card")}</Modal.Title>
      </Modal.Header>
      <Modal.Body className="pt-2">
        <PaymentForm cardHolderName={cardHolderName} onCardHolderNameChange={setCardHolderName} isSubmitting={isSubmitting} error={error} hideHeader={true} stripeComponents={stripeComponents} />
      </Modal.Body>
      <Modal.Footer className="border-0 pt-0 d-flex justify-content-end align-items-center">
        ...
        <Button variant="primary" onClick={handleLocalSubmit} disabled={isSubmitting || !stripe} className="px-4 py-2 fw-bold">
          {isSubmitting ? <Spinner animation="border" size="sm" /> : t("Save Card")}
        </Button>
      </Modal.Footer>
    </div>;
};
...
const AddCardModal = ({
  ...
  return <Modal show={show} onHide={onHide} centered backdrop="static" size="lg">
      {stripeComponents && <stripeComponents.Elements stripe={stripePromise}>
          <AddCardStripeWrapper onHide={onHide} onAddSuccess={onAddSuccess} apiBase={apiBase} token={token} onToast={onToast} stripeComponents={stripeComponents} paymentMethods={paymentMethods} />
...
const PaymentMethodsSection = ({
  ...
  const handleSetDefault = async pmId => {
    ...
    try {
      const res = await fetch(`${apiBase}/api/subscription/payment-methods/set-default`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json"
      ...
      onToast?.(t("default_card_updated") || "Default payment method updated", "success");
      onUpdate(data);
    } catch (err) {
  ...
  const handleRemove = async (e, pmId) => {
    e.stopPropagation();
    if (!window.confirm(t("confirm_remove_card") || "Remove this card?")) return;
    setIsRemoving(pmId);
    try {
      const res = await fetch(`${apiBase}/api/subscription/payment-methods/${pmId}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`
        }
      ...
      onUpdate(data);
      onToast?.(t("card_removed_success") || "Card removed successfully!", "success");
    } catch (err) {
      onToast?.(err.message, "error");
    } finally {
      setIsRemoving(null);
  ...
  return <div className="st-card st-payments-wrapper">
      ...
      <div className="st-pm-grid">
        ...
        return <div key={m.id} className={`st-pm-item ${isDefault ? "st-pm-active" : ""} ${isLoading ? "st-pm-busy" : ""}`} onClick={() => handleSetDefault(m.id)}>
              <div className="st-pm-content">
                ...
                <div className="st-pm-info">
                  ...
                  <div className="st-pm-meta">
                    <span className="st-pm-brand subscription-tab--s1">
                      {m.brand}
                    </span>
                ...
                {isDefault && <div className="st-pm-check">
                    <Check size={14} strokeWidth={3} />
                  </div>}
              </div>

              {!isDefault && <button className="st-pm-remove-btn" onClick={e => handleRemove(e, m.id)} disabled={isLoading}>
                  {isRemoving === m.id ? <Spinner animation="border" size="sm" /> : <Trash2 size={13} />}
                </button>}

              {isLoading && <div className="st-pm-busy-overlay">
                  <Spinner animation="border" size="sm" variant="primary" />
                </div>}
            </div>;
      })}

        {paymentMethods.length === 0 && <div className="st-pm-empty">
            <CreditCard size={32} />
            <p>{t("no_payment_methods") || "No payment methods added yet"}</p>
          </div>}
      </div>

      <AddCardModal show={isAdding} onHide={() => setIsAdding(false)} onAddSuccess={onUpdate} apiBase={apiBase} token={token} onToast={onToast} paymentMethods={paymentMethods} />
    </div>;
};
const SubscriptionTab = ({
  ...
  const billingColumns = [{
    ...
    key: "plan",
    label: t("plan") || "Plan",
    render: (_, row) => <span className="admin-cell-primary subscription-tab--s1">
          {row.plan_name} plan
        </span>
  }, {
    key: "amount",
    label: t("amount") || "Amount",
    render: (_, row) => <span className="admin-cell-primary">${row.amount}.00</span>
  }];
  const accessFeatures = [{
    ...
    active: usage.projects?.original_limit ?? usage.projects?.limit ?? usage.project
  }];
  ...
  return <div className="st-root">
      <div className="st-disclaimer-banner">
        <Alert variant="info" className="d-flex align-items-center py-2 px-3 border-0 shadow-sm subscription-tab--s2">
          <AlertCircle size={18} className="me-2 flex-shrink-0" />
          <div className="subscription-tab--s3">
      ...
      {}
      {loading && <div className="admin-loading-bar-container subscription-tab--s4">
      ...
      <div className="st-header-clean">
        <div className="st-header-main">
          ...
          <div className="st-header-stats">
            {!is_unlimited && end_date && <div className="st-h-stat">
      ...
      <div className="st-body-grid">
        ...
        <div className="st-card st-access-card">
          ...
          <div className="st-access-list">
            ...
            desc,
            active
          }) => <div key={label} className={`st-access-item ${active ? "st-ai-on" : "st-ai-off"}`}>
                <div className={`st-ai-icon-wrap ${active ? "st-ai-icon-on" : "st-ai-icon-off"}`}>
                  <Icon size={16} />
                </div>
                ...
                <div className={`st-ai-status ${active ? "st-ais-on" : "st-ais-off"}`}>
                  {active ? <>
        ...
        {}
        {currentPlanData && <div className="st-card st-plan-summary-card">
            <div className="st-card-header">
              <div className="st-card-title">Plan Details</div>
              <div className="st-card-subtitle subscription-tab--s1">
            ...
            icon: Eye,
            label: "Viewers",
            val: displayLimits.viewers
          }, {
            icon: Users,
            label: "Users",
            val: displayLimits.users
          }].map(({
            icon: Icon,
            label,
            val
      ...
      {}
      <PaymentMethodsSection paymentMethods={payment_methods} defaultId={default_payment_method_id} apiBase={API_BASE_URL} token={token} onUpdate={handlePaymentMethodsUpdate} onToast={onToast} />
      ...
      {}
      <AdminTable title={t("billing_history") || "Billing History"} count={totalItems} countLabel={t("Records") || "Records"} columns={billingColumns} data={paginatedHistory} currentPage={currentPage} totalPages={totalPages} onPageChange={setCurrentPage} totalItems={totalItems} itemsPerPage={itemsPerPage} loading={loading} emptyMessage={t("no_billing_history") || "No billing history found"} />

      <UpgradeModal show={showUpgradeModal} onHide={() => setShowUpgradeModal(false)} availablePlans={available_plans} currentPlanName={currentPlanName} paymentMethod={payment_methods} initialPlanId={selectedPlanId} onUpgradeSuccess={updatedSub => {
      queryClient.invalidateQueries({
        queryKey: ["planDetails"]
      });
      queryClient.invalidateQueries({
