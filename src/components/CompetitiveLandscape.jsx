...
import { Loader, ChevronDown, ChevronRight, Users, Shield, Zap } from 'lucide-react';
...
const CompetitiveLandscape = ({
  ...
  const handleMissingQuestionsCheck = useCallback(async () => {
    ...
    await checkMissingQuestionsAndRedirect('competitiveLandscape', selectedBusinessId, handleRedirectToBrief, {
  ...
  return <div className="porters-container full-swot-container" data-analysis-type="competitiveLandscape" data-analysis-name="Competitive Landscape" data-analysis-order="9" data-component="competitive-landscape">
            ...
            {ourDifferentiators.length > 0 && <div className="section-container">
                    <div className="section-header competitive-landscape--s1" onClick={() => toggleSection('differentiators')}>
                        <div className="competitive-landscape--s2">
                            <Shield size={20} color="#6366f1" />
                            <h5 className="competitive-landscape--s3">Our Differentiators vs Competitors</h5>
                        </div>
              ...
              return <StreamingRow key={index} isVisible={isVisible} isLast={rowIndex === visibleRows - 1 && isStreaming} lastRowRef={lastRowRef} isStreaming={isStreaming}>
                                                <td className="competitive-landscape--s4">{diff.differentiator}</td>
                                                <td className="competitive-landscape--s5">
                                                    {hasStreamed ? diff.description : typingTexts[`${rowIndex}-item`] || diff.description}
                                                </td>
                                                <td className="competitive-landscape--s5">
                                                    <div className="competitive-landscape--s6">
                  ...
                  }} className="competitive-landscape--s7">
                                                        {diff.sustainability}
                                                    </span>
                                                </td>
                                            </StreamingRow>;
      ...
      return <div key={competitorIndex} className="section-container">
                        <div className="section-header" onClick={() => toggleSection(competitor)}>
                            <div className="competitive-landscape--s2">
                                <Users size={20} color="#6366f1" />
                                <h5 className="competitive-landscape--s3">{competitor}</h5>
                            </div>
                ...
                return Array.isArray(categoryData) ? categoryData.map((item, itemIndex) => {
                  const key = `${competitor}-${category}-${itemIndex}`;
                  const rowIndex = swotRowIndices[key];
                  const isVisible = rowIndex < visibleRows;
                  return <StreamingRow key={key} isVisible={isVisible} isLast={rowIndex === visibleRows - 1 && isStreaming} lastRowRef={lastRowRef} isStreaming={isStreaming}>
                                                            {itemIndex === 0 && <td rowSpan={categoryData.length}><span className={`status-badge ${categoryColor}`}>{categoryLabel}</span></td>}
