...
const ChannelEffectivenessMap = ({
  ...
  const BubbleChart = ({ data }) => { 
    if (!data || data.length === 0) {
      return (
        <div style={{
          width: 600, height: 400, display: 'flex', alignItems: 'center', justifyContent: 'center',
          border: '2px dashed #d1d5db', borderRadius: '12px', backgroundColor: '#f9fafb'
        }}>
          <div style={{ textAlign: 'center', color: '#6b7280' }}>
            <BarChart3 size={48} style={{ margin: '0 auto 12px', display: 'block' }} />
            <div style={{ fontSize: '16px', fontWeight: '600', marginBottom: '4px' }}>No Valid Channel Data</div>
            <div style={{ fontSize: '14px' }}>Unable to display chart with current data</div>
          </div>
        </div>
      );
    }
      ...
      <div className="bubble-chart-container" ref={bubbleChartRef} style={{ position: 'relative' }}>
          ...
          <g transform={`translate(${margin.left}, ${margin.top})`}> 
            ...
            {data.map((channel) => {
              ...
              return (
                <g key={channel.name}>
                  <circle
                    cx={x} cy={y} r={radius} fill={channel.color} fillOpacity="0.85"
                    stroke={isHovered ? '#ffffff' : channel.color} strokeWidth="3"
                    style={{ cursor: 'pointer', transition: 'all 0.2s ease', filter: 'drop-shadow(0 4px 8px rgba(0, 0, 0, 0.2))', transform: isHovered ? 'scale(1.1)' : 'scale(1)' }}
                    onMouseMove={(e) => handleMouseMove(e, channel)}
                    onMouseLeave={handleMouseLeave}
                  />
                  <text x={x} y={y + 4} textAnchor="middle" fontSize="12" fontWeight="700" fill="#1f2937" style={{ pointerEvents: 'none', textShadow: '1px 1px 2px rgba(255, 255, 255, 0.8)' }}>
                    {channel.name}
                  </text>
                  {channel.trend !== 'stable' && (
                    <text x={x + radius + 8} y={y - radius - 5} fontSize="16" fill={channel.trend === 'increasing' ? '#00ff88' : '#ff4757'} style={{ pointerEvents: 'none', filter: 'drop-shadow(0 2px 4px rgba(0, 0, 0, 0.3))' }}>
        ...
        {hoveredChannel && (
          <div style={{
              ...
              background: 'rgba(0, 0, 0, 0.9)', color: 'white', padding: '12px 16px', borderRadius: '8px',
              fontSize: '12px', maxWidth: '200px', zIndex: 10, boxShadow: '0 4px 20px rgba(0, 0, 0, 0.3)',
              pointerEvents: 'none', whiteSpace: 'nowrap', transform: tooltipPosition.x > 400 ? 'translateX(-100%)' : 'none'
            }}>
            <div style={{ fontWeight: '700', marginBottom: '6px', fontSize: '13px' }}>{hoveredChannel.name}</div>
            <div style={{ margin: '3px 0', fontSize: '11px' }}>Effectiveness: {hoveredChannel.effectiveness.toFixed(1)}</div>
            <div style={{ margin: '3px 0', fontSize: '11px' }}>Efficiency: {hoveredChannel.efficiency.toFixed(1)}</div>
            <div style={{ margin: '3px 0', fontSize: '11px' }}>Revenue: {hoveredChannel.revenue.toFixed(1)}%</div>
            {hoveredChannel.trend !== 'stable' && (
              <div style={{ marginTop: '6px', fontWeight: '600', fontSize: '12px', color: hoveredChannel.trend === 'increasing' ? '#00ff88' : '#ff4757' }}>Trend: {hoveredChannel.trend}</div>
            )}
          </div>
        )}
      </div>
