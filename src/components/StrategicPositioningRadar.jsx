...
const StrategicPositioningRadar = ({
  ...
  const handleMissingQuestionsCheck = useCallback(async () => {
    ...
    await checkMissingQuestionsAndRedirect('strategicRadar', selectedBusinessId, handleRedirectToBrief, {
  ...
  const isStrategicRadarDataIncomplete = useCallback(radarData => {
    ...
    return !strategicRadar.dimensions.some(dimension => dimension.name && (dimension.currentScore !== undefined || dimension.targetScore !== undefined));
  }, []);
  ...
  if (isRegenerating) {
    return <div className="strategic-radar-container">
  ...
  if (!data || isStrategicRadarDataIncomplete(data)) {
    return <div className="strategic-radar-container">
                <AnalysisEmptyState analysisType="strategicRadar" analysisDisplayName="Strategic Positioning Radar Analysis" icon={Target} onImproveAnswers={handleMissingQuestionsCheck} onRegenerate={handleRegenerate} isRegenerating={isRegenerating} canRegenerate={canRegenerate} userAnswers={userAnswers} minimumAnswersRequired={3} showImproveButton={false} showRegenerateButton={false} />
            </div>;
  }
  const {
    strategicRadar
  ...
  return <div className="strategic-radar-container">
            <div className="competitive-advantage-content">
                    <div className="overview-content">
                        {overallPosition && <div className="section-container">
                                        ...
                                        <table className="data-table">
                                            ...
                                            <tbody>
                                                <StreamingRow isVisible={executiveIndices.currentAverage < visibleRows} isLast={executiveIndices.currentAverage === visibleRows - 1 && isStreaming} lastRowRef={lastRowRef} isStreaming={isStreaming}>
                                                    ...
                                                    <td style={{
                      opacity: executiveIndices.currentAverage < visibleRows ? 1 : 0,
                      transition: !isStreaming ? 'none' : 'opacity 0.3s 0.4s'
                    }}>
                                                ...
                                                <StreamingRow isVisible={executiveIndices.targetAverage < visibleRows} isLast={executiveIndices.targetAverage === visibleRows - 1 && isStreaming} lastRowRef={lastRowRef} isStreaming={isStreaming}>
                                                    ...
                                                    <td style={{
                      opacity: executiveIndices.targetAverage < visibleRows ? 1 : 0,
                      transition: !isStreaming ? 'none' : 'opacity 0.3s 0.4s'
                    }}>
                                                ...
                                                <StreamingRow isVisible={executiveIndices.improvementGap < visibleRows} isLast={executiveIndices.improvementGap === visibleRows - 1 && isStreaming} lastRowRef={lastRowRef} isStreaming={isStreaming}>
                                                    ...
                                                    <td style={{
                      opacity: executiveIndices.improvementGap < visibleRows ? 1 : 0,
                      transition: !isStreaming ? 'none' : 'opacity 0.3s 0.4s'
                    }}>
                                        ...
                                        {overallPosition.strengthAreas && <div className="subsection"><h4>{t("strategic_card1_head4")}</h4><div className="forces-tags">{overallPosition.strengthAreas.map((area, index) => <span key={index} className="force-tag">{area}</span>)}</div></div>}
                                        {overallPosition.improvementAreas && <div className="subsection"><h4>{t("strategic_card1_head5")}</h4><div className="forces-tags">{overallPosition.improvementAreas.map((area, index) => <span key={index} className="force-tag">{area}</span>)}</div></div>}
                                    </div>
                                </div>
                            </div>}
                        {dimensions && <div className="section-container">
                    ...
                    return <StreamingRow key={index} isVisible={isVisible} isLast={rowIndex === visibleRows - 1 && isStreaming} lastRowRef={lastRowRef} isStreaming={isStreaming}>
                        ...
                        }}>{hasStreamed ? dimension.currentScore : typingTexts[`${rowIndex}-current`] || dimension.currentScore}</span></td>
                                                            <td style={{
                        opacity: isVisible ? 1 : 0,
                        transition: !isStreaming ? 'none' : 'opacity 0.3s 0.5s'
                      }}><span className="score-badge strategic-positioning-radar--s1" style={{
                          backgroundColor: getScoreColor(dimension.targetScore)
                        }}>{dimension.targetScore}</span></td>
                                                            <td style={{
                        opacity: isVisible ? 1 : 0,
                        transition: !isStreaming ? 'none' : 'opacity 0.3s 0.6s'
                      }}><span className="score-badge strategic-positioning-radar--s2">{dimension.industryAverage}</span></td>
                                                        </StreamingRow>;
                  })}
                                            </tbody>
                                        </table>
