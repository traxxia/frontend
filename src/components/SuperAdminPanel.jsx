...
const SuperAdminPanel = () => {
  ...
  const allTabs = [{
    id: "companies",
    label: isSuperAdmin ? t('companies') : t('company'),
    icon: Building2
  }, {
    id: "businesses",
    label: t('businesses') || "Businesses",
    icon: Briefcase
  }, {
    id: "user_management",
  ...
  return <div className="super-admin-container">
      ...
      <div className="admin-header">
        <div className="admin-header-content">
          <div className="header-left">
            {!isSuperAdmin && <button className="back-button" onClick={handleBack}>
                <ArrowLeft size={18} />
              </button>}
      ...
      <div className="admin-nav super-admin-panel--s1">
        {tabs.map(tab => {
        const IconComponent = tab.icon;
        return <button key={tab.id} className={`nav-tab ${activeTab === tab.id ? "active" : ""}`} onClick={() => {
          navigate(`?tab=${tab.id}`, {
            replace: true
          });
