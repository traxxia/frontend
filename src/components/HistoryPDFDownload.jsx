...
const HistoryPDFDownload = ({
  ...
  const getExportPhase = () => {
    ...
    const hasGoodPhaseData = !!(analysisData.costEfficiency || analysisData.financialPerformance || analysisData.financialBalance || analysisData.operationalEfficiency);
    if (hasGoodPhaseData) return 'good';
    const hasEssentialPhaseData = !!(analysisData.fullSwot || analysisData.customerSegmentation || analysisData.competitiveAdvantage || analysisData.channelEffectiveness || analysisData.expandedCapability || analysisData.strategicGoals || analysisData.strategicRadar || analysisData.cultureProfile || analysisData.productivityMetrics || analysisData.maturityScore);
  ...
  const phaseComponents = {
    initial: [{
      ...
      selector: '[data-component="channel-heatmap"]',
      name: 'Channel Heatmap'
    }, {
      selector: '[data-component="loyalty-nps"]',
      name: 'Loyalty & NPS Analysis'
    }, {
      selector: '[data-component="capability-heatmap"]',
    ...
    essential: [{
      ...
      selector: '[data-component="customer-segmentation"]',
      name: 'Customer Segmentation'
    }, {
      selector: '[data-component="competitive-advantage"]',
      name: 'Competitive Advantage Matrix'
    }, {
      selector: '[data-component="channel-effectiveness"]',
      name: 'Channel Effectiveness Map'
    }, {
      selector: '[data-component="expanded-capability"]',
      name: 'Capability Heatmap'
    }, {
      selector: '[data-component="strategic-goals"]',
      name: 'Strategic Goals'
    }, {
      selector: '[data-component="strategic-radar"]',
      name: 'Strategic Positioning Radar'
    }, {
      selector: '[data-component="culture-profile"]',
      name: 'Organizational Culture Profile'
    }, {
    ...
    good: [{
      ...
      selector: '[data-component="cost-efficiency"]',
      name: 'Cost Efficiency Insight'
    }, {
      selector: '[data-component="financial-performance"]',
      name: 'Financial Performance & Growth Trajectory'
    }, {
      selector: '[data-component="financial-health"]',
      name: 'Financial Health Insight'
    }, {
      selector: '[data-component="operational-efficiency"]',
      name: 'Operational Efficiency Insight'
    }]
  };
  ...
  const handleFastExport = async () => {
    ...
    try {
      ...
      pdf.text(phaseTitle, pageWidth / 2, 30, {
        align: 'center'
      });
      pdf.setFontSize(16);
      pdf.setTextColor(0, 0, 0);
      pdf.text(businessName, pageWidth / 2, 45, {
        align: 'center'
      });
      pdf.setFontSize(12);
      pdf.setTextColor(128, 128, 128);
      pdf.text(`Generated on ${new Date().toLocaleDateString()}`, pageWidth / 2, 55, {
        align: 'center'
      });
      await addConversationHistory(pdf, pageWidth, pageHeight);
      const components = phaseComponents[exportPhase].filter(({
        selector
      }) => {
        const element = document.querySelector(selector);
        return element && element.offsetHeight > 50 && element.offsetWidth > 50 && !element.closest('.collapsed');
      });
      ...
      for (let i = 0; i < components.length; i += 3) {
        ...
        const results = await Promise.all(batch.map(({
          selector,
          name
        }) => captureComponent(selector, name)));
  ...
  return <>
      {}
      {isExporting && <div className="history-p-d-f-download--s1">
          <div className="history-p-d-f-download--s2">
            <Loader size={32} className="history-p-d-f-download--s3" />
            <h3 className="history-p-d-f-download--s4">
              Generating PDF
            </h3>
            <p className="history-p-d-f-download--s5">
              Creating analysis for {businessName}...
            </p>
          </div>
        </div>}

      <button onClick={handleFastExport} disabled={isExporting || !analysisData} className={`${className} history-p-d-f-download--s6`} style={{
      ...
      ...style
        ...
        {isExporting ? <>
            <Loader size={16} className="history-p-d-f-download--s7" />
            Generating...
          </> : <>
            <Download size={16} />
            {t("Export_PDF")}
