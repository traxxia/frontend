...
const TeamRankingsView = ({
  ...
  const renderCollaboratorDetails = consensus => {
    ...
    return <div className="collaborator-details-panel">
        <h6 className="collab-header">Team Rankings Summary</h6>
        <div className="table-responsive">
          <Table size="sm" className="collab-table mb-0">
            <thead>
              <tr>
                <th className="team-rankings-view--s1">Collaborator</th>
                <th className="text-center team-rankings-view--s2">Rank</th>
                <th>Rationale</th>
              </tr>
            </thead>
            <tbody>
              {collabData.map((collab, idx) => <tr key={idx}>
                  <td>
  ...
  return <div className="ranking-panel-container expanded-view">

      {}
      <div className="ranking-content-body">
        {}
        {isSuperAdmin && <div className="ranking-toolbar">
            <div className="consensus-toggle-group">
              <button className={`toggle-btn ${consensusMode === "ai" ? "active" : ""}`} onClick={() => setConsensusMode("ai")}>
                <Sparkles size={14} /> {t("ai_consensus")}
              </button>
              <button className={`toggle-btn ${consensusMode === "collaborator" ? "active" : ""}`} onClick={() => setConsensusMode("collaborator")}>
                <Users size={14} /> {t("team_consensus")}
              </button>
            </div>

            {}
            {consensusSummary && consensusMode === "ai" && <div className="mini-summary">
        ...
        {loadingConsensus ? <div className="loading-state">
            <Spinner animation="border" size="sm" />
            <span>Analyzing...</span>
          </div> : <div className="compact-table-wrapper">
            <Table hover size="sm" className="ranking-table">
              {consensusMode === "collaborator" && isSuperAdmin && <colgroup>
                  <col className="team-rankings-view--s3" />
                  <col className="team-rankings-view--s4" />
                </colgroup>}
              <thead>
                <tr>
                  {consensusMode === "collaborator" && isSuperAdmin ? <th colSpan={2} className="team-rankings-view--s5">{t("project_name")}</th> : <th>{t("project_name")}</th>}
                  {!isSuperAdmin && userRole !== 'viewer' && <th className="text-center">{t("my_rank")}</th>}
                  {consensusMode === "ai" && <th className="text-center">{t("ai_rank")}</th>}
                  {isSuperAdmin && consensusMode === "ai" && <th className="text-center team-rankings-view--s6">{t("consensus")}</th>}
                </tr>
              </thead>
              <tbody>
              ...
              return <React.Fragment key={p._id}>
                      <tr className={`ranking-row ${isExpanded ? 'expanded' : ''}`} onClick={() => hasCollaboratorData && consensusMode === 'collaborator' && toggleRowExpansion(key)}>
                        {consensusMode === "collaborator" && isSuperAdmin && (hasCollaboratorData ? <td className="text-center toggle-cell">
                              {isExpanded ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                            </td> : null)}
                        <td className="project-name-cell" colSpan={consensusMode === "collaborator" && isSuperAdmin && !hasCollaboratorData ? 2 : 1}>
                          <div className="name-wrapper team-rankings-view--s7">
                            {p.project_name}
                          </div>
                        </td>     {!isSuperAdmin && userRole !== 'viewer' && <td className="text-center">
                            {userRank ? <Badge bg="primary" className="rank-badge">{userRank}</Badge> : <span className="text-muted">-</span>}
                          </td>}

                        {consensusMode === "ai" && <td className="text-center">
                            {aiRank ? <Badge bg="warning" text="dark" className="rank-badge ai">{aiRank}</Badge> : <span className="text-muted">-</span>}
                          </td>}

                        {isSuperAdmin && consensusMode === "ai" && <td className="text-center">
                            {consensus ? <div className="d-flex align-items-center justify-content-center gap-2">
                                <span>{getConsensusEmoji(consensus.consensus_score)}</span>
                                <span className="small fw-bold" style={{
                        color: consensus.consensus_score === 'green' ? '#10b981' : consensus.consensus_score === 'yellow' ? '#f59e0b' : consensus.consensus_score === 'red' ? '#ef4444' : '#64748b'
                      }}>
                                </span>
                              </div> : "-"}
                          </td>}
                      </tr>
                      {consensusMode === "collaborator" && isSuperAdmin && hasCollaboratorData && isExpanded && <tr className="details-row">
                          <td colSpan={10}>
