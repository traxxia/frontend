...
const AcademyFeedbackAdmin = ({
  ...
  const columns = [{
    ...
    render: (val, row) => {
      ...
      if (!dateVal) return '-';
      return <div className="admin-cell-secondary">
                        {formatDate(dateVal)}
                    </div>;
    }
  }, {
    ...
    render: val => {
      if (val === true) {
        return <span className="admin-status-badge launched admin-icon-badge">
                            <ThumbsUp size={12} /> Yes
                        </span>;
      }
      return <span className="admin-status-badge inactive admin-icon-badge">
    ...
    render: val => {
      ...
      if (typeof val === 'object' && val !== null) {
        return <div className="admin-cell-primary admin-text-sm">
      ...
      return <div className="admin-cell-secondary admin-text-sm">
    ...
    render: val => {
      ...
      return <Button variant="outline-primary" size="sm" className="admin-text-xs d-flex align-items-center gap-1" onClick={() => handleShowModal(val)} title="View full feedback">
                        <Eye size={12} /> View Feedback
                    </Button>;
    }
  }];
  const MyFilterToolbar = <div className="admin-filter-toolbar">
            <span className="admin-filter-label">Filter by:</span>
            <select value={filterHelpful} onChange={e => setFilterHelpful(e.target.value)} className="admin-filter-select">
                <option value="all">All Feedback</option>
                <option value="yes">Helpful (Yes)</option>
                <option value="no">Not Helpful (No)</option>
            </select>
        </div>;
  return <div className="academy-feedback-admin admin-fade-in">
            <AdminTable title="Academy Feedback" columns={columns} data={paginatedData} loading={isLoading} searchTerm={searchTerm} onSearchChange={setSearchTerm} searchPlaceholder="Search by article..." toolbarContent={MyFilterToolbar} totalItems={totalItems} currentPage={currentPage} totalPages={totalPages} itemsPerPage={itemsPerPage} onPageChange={setCurrentPage} emptyMessage={filterHelpful !== 'all' || searchTerm ? "No feedback matches your filter" : "No feedback collected yet"} />

            {}
            <Modal show={showModal} onHide={handleCloseModal} centered scrollable className="admin-compact-modal" contentClassName="admin-feedback-modal-content">
                ...
                <Modal.Body className="admin-modal-body p-4">
                    <div className="admin-feedback-content-full">
                        <p className="admin-text-primary admin-lh-base pre-wrap academy-feedback-admin--s1">
                            {selectedFeedback}
                        </p>
                    </div>
                </Modal.Body>
