...
const ProjectDetails = ({
    ...
    const fetchDecisionLogs = async (targetPage = 1) => {
    ...
    }, [project?._id, page, sortOrder, logTypeFilter, executionStateFilter]);
    ...
    const handleCreateManualLog = async () => {
        ...
        try {
            await decisionLogApiService.createDecisionLog(project._id, {
                ...
                justification,
                metadata: {
                    source: "project_details_manual_form"
                }
            });
            setNewJustification("");
            ...
            <div className="details-card mt-4">
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "10px", marginBottom: "12px" }}>
                    <select className="form-select" value={logTypeFilter} onChange={(e) => { setLogTypeFilter(e.target.value); setPage(1); }}>
                        ...
                        <option value="manual">{t("Manual") || "Manual"}</option>
                        <option value="status_change">{t("Status") || "Status Change"}</option>
                        <option value="review_update">{t("Review") || "Review"}</option>
                        <option value="adhoc_update">{t("Ad_Hoc_Update") || "Ad Hoc Update"}</option>
                    </select>
                        ...
                        <table className="table">
                            ...
                            <tbody>
                                {decisionLogs.length === 0 ? (
                                    <tr>
                                        <td colSpan={6}>{t("No_decision_logs_available") || "No decision logs available"}</td>
                                    </tr>
                                ) : (
                                    decisionLogs.map((log) => (
                                        <tr key={String(log._id)}>
                                            ...
                                            <td>
                                                <button className="btn-edit" onClick={() => handleOpenDetails(log._id)} style={{ padding: "4px 10px", fontSize: "12px" }}>
                                                    {t("View") || "View"}
                                                </button>
                                            </td>
                                        </tr>
