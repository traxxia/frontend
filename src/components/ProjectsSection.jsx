...
const ProjectsSection = ({
  ...
  const {
    locks
  } = useFieldLockPolling(currentProject?._id, activeView === "edit");
  const {
    userHasRerankAccess,
  ...
  const isLockedByOther = useCallback(field => locks.some(l => l.field_name === field && l.locked_by !== myUserId), [locks, myUserId]);
  const getLockOwnerForField = useCallback(field => {
    const lock = locks.find(l => l.field_name === field && l.locked_by !== myUserId);
  ...
  const executeLaunchProjects = useCallback(async () => {
    try {
      ...
      const {
        ...
        error,
        data
      } = await launchProjects(selectedProjectIds);
      if (success) {
        addToast({
  ...
  const submitReview = useCallback(async data => {
    ...
    try {
      const {
        success,
        error
      } = reviewType === "review" ? await reviewProjectAction(selectedReviewProject._id, data) : await adhocUpdateProjectAction(selectedReviewProject._id, data);
      if (success) {
  ...
  const renderProjectList = () => {
    ...
    const selectedCategoryLabel = isAllSelected ? t("all") : selectedCategories.length === 1 ? t(CATEGORIES.find(c => c.id === selectedCategories[0])?.label || "all") : `${selectedCategories.length} ${t("selected")}`;
    const totalCount = isAllSelected ? projects.length : projects.filter(p => {
      const statusValue = (p.status || "Draft").toLowerCase();
