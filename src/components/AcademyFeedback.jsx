...
const AcademyFeedback = ({
  ...
  if (submitted) {
    return <div className="academy-feedback academy-feedback-submitted">
  ...
  return <div className="academy-feedback">
            <div className="feedback-question">Was this article helpful?</div>

            <div className="feedback-buttons">
                <button className={`feedback-btn feedback-btn-yes ${feedbackType === 'yes' ? 'active' : ''}`} onClick={() => handleFeedback('yes')} disabled={feedbackType !== null}>
                    👍 Yes
                </button>
                <button className={`feedback-btn feedback-btn-no ${feedbackType === 'no' ? 'active' : ''}`} onClick={() => handleFeedback('no')} disabled={feedbackType !== null}>
                    👎 No
                </button>
            </div>

            {showTextarea && <div className="feedback-textarea-wrapper">
                    <textarea className="feedback-textarea" placeholder={feedbackType === 'yes' ? "What did you like about it? (optional)" : "What could we improve? (optional)"} value={feedbackText} onChange={e => {
        setFeedbackText(e.target.value);
        if (error) setError('');
      }} rows={3} />
                    {error && <div className="feedback-error-message academy-feedback--s1">{error}</div>}
                    <button className="feedback-submit-btn" onClick={handleSubmitText}>
                        Submit Feedback
                    </button>
                </div>}
        </div>;
