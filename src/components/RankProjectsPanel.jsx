...
import { Button, Card, Accordion, useAccordionButton, Row, Col, Alert, Modal, Form } from "react-bootstrap";
...
const isMandatoryForCollaborator = p => (p.ai_rank !== null && p.ai_rank !== undefined || p.is_admin_ranked || isLaunched(p) || isActiveStatus(p)) && !isTerminalStatus(p);
...
function RationaleToggle({
  ...
  return <Button variant="link" onClick={decoratedOnClick} className="rank-rationale-toggle">
      {children}
    </Button>;
}
const RankProjectsPanel = ({
  show,
  projects,
  onLockRankings,
  onRankSaved,
  ...
  useEffect(() => {
    ...
    if (!hasInitialized.current) {
      if (isAdmin) {
        const initialRankedIds = projects.filter(p => !isLaunched(p) && p.rank !== null && p.rank !== undefined).map(p => p._id);
        setSelectedDraftIds(initialRankedIds);
        const allEligible = projects.filter(p => !isKilled(p));
        const unranked = allEligible.filter(p => (isMandatoryOrActive(p) || initialRankedIds.includes(p._id)) && (p.rank === null || p.rank === undefined));
    ...
    if (step === 2 || !isAdmin) {
      const listToShow = isAdmin ? projects.filter(p => (isMandatoryOrActive(p) || selectedDraftIds.includes(p._id)) && !isTerminalStatus(p)) : projects.filter(isMandatoryForCollaborator);
      const sorted = [...listToShow].sort((a, b) => {
        const rankA = a.rank !== null && a.rank !== undefined ? a.rank : a.ai_rank !== null && a.ai_rank !== undefined ? a.ai_rank : Infinity;
        const rankB = b.rank !== null && b.rank !== undefined ? b.rank : b.ai_rank !== null && b.ai_rank !== undefined ? b.ai_rank : Infinity;
  ...
  const rankedDraftIds = useMemo(() => projects.filter(p => !isLaunched(p) && p.rank !== null && p.rank !== undefined).map(p => p._id), [projects]);
  ...
  const handleToggleDraft = projectId => {
    setSelectedDraftIds(prev => prev.includes(projectId) ? prev.filter(id => id !== projectId) : [...prev, projectId]);
  };
  const handleNextToRanking = async () => {
    ...
    try {
      const uniqueProjects = selectedProjects.filter((project, index, self) => index === self.findIndex(p => p._id === project._id));
      const {
        callMLRankingAPI,
        saveAIRankings,
        fetchTeamRankings
      ...
      const {
        success,
        rankings
      } = await callMLRankingAPI(uniqueProjects);
      if (success && rankings && rankings.length > 0) {
        ...
        const validRankings = rankedList.filter(p => p.rank !== Infinity && p.rank !== null && p.rank !== undefined).map(p => ({
          project_id: p._id,
          rank: p.rank
        }));
        if (validRankings.length > 0) {
          await saveAIRankings(businessId, validRankings);
          fetchTeamRankings(businessId, {
            silent: true
          });
        }
  ...
  const renderStep1 = () => <div className="rank-step-selection">
      <h6 className="mb-3 text-primary">{t("Launched Bets (Mandatory Bets)")}</h6>
      <div className="selection-list mb-4">
        {activeProjects.map(p => <div key={p._id} className="selection-item mandatory">
            <Form.Check type="checkbox" checked={true} disabled={true} label={p.project_name} />
          </div>)}
      ...
      <div className="selection-list mb-4">
        {draftProjects.map(p => <div key={p._id} className="selection-item clickable-row" onClick={() => !isArchived && !isGeneratingAI && handleToggleDraft(p._id)} style={{
        cursor: isArchived || isGeneratingAI ? "not-allowed" : "pointer"
      }}>
            <Form.Check type="checkbox" id={`draft-${p._id}`} checked={selectedDraftIds.includes(p._id)} onChange={() => !isGeneratingAI && handleToggleDraft(p._id)} onClick={e => e.stopPropagation()} label={p.project_name} disabled={isArchived || isGeneratingAI} />
          </div>)}
  ...
  const renderStep2 = () => <>
      {projectList.length > 0 && <div className={`d-flex ${isAdmin && !initialAllRanked ? "justify-content-end" : "justify-content-end"} align-items-center mb-3`}>
      ...
      {}
      {validationError && <Alert variant="danger" dismissible onClose={() => setValidationError("")} className="mt-3">
      ...
      <DragDropContext onDragEnd={handleDragEnd}>
        <Droppable droppableId="rank-projects">
          {provided => <Accordion alwaysOpen ref={provided.innerRef} {...provided.droppableProps}>
              {projectList.length === 0 ? <div className="text-center py-5 text-muted border rounded bg-light">
                  <p className="mb-0">{t("No projects have been prioritized for ranking by the admin yet.")}</p>
                </div> : projectList.map((item, index) => {
            ...
            return <Draggable key={item._id} draggableId={item._id} index={index} isDragDisabled={isArchived || !isAdmin && !userHasRerankAccess && !userHasRankingAccess}>
                      {(provided, snapshot) => <Card ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps} className={`rank-project-card responsive-card ${snapshot.isDragging ? "dragging" : ""} ${needsRationale ? "needs-rationale" : ""}`}>
                          <Card.Body>
                            {}
                            <div className="rank-card-top responsive-card-top">
                              ...
                              <div className="rank-content">
                                <div className="d-flex align-items-center gap-2">
                                  ...
                                  {needsRationale && <AlertTriangle size={16} color="#dc3545" title="Rationale required" />}
                                </div>
                              ...
                              <div className="rank-move-buttons responsive-move-buttons" style={{
                      cursor: !isAdmin && !userHasRerankAccess && !userHasRankingAccess ? "not-allowed" : "grab"
                    }}>
                                <ChevronUp size={18} />
                                <ChevronDown size={18} />
                              </div>
                            ...
                            <div className="rank-rationale-btn-container responsive-rationale-btn">
                              <RationaleToggle eventKey={index.toString()}>
                                {item.rationale && item.rationale.trim() !== "" ? positionChanged ? <span style={{
                        color: needsRationale ? "#dc3545" : "#28a745"
                      }}>
                                      {validation.isValid ? t("Edit Rationale ▼") : t("⚠️ Edit Rationale (Invalid) ▼")}
                                    </span> : <span className="rank-projects-panel--s1">
                                      {t("Edit Rationale ▼")}
                                    </span> : positionChanged ? <span className="rank-projects-panel--s2">
                                      ⚠️ {t("Add Rationale (Required) ▼")}
                                    </span> : t("Add Rationale ▼")}
                              </RationaleToggle>
                            </div>

                            <Accordion.Collapse eventKey={index.toString()}>
                              <div className="mt-2">
                                {item.rationale && item.rationale.includes("<strong>") ? <div className="rank-rationale-display responsive-textarea rank-projects-panel--s3" dangerouslySetInnerHTML={{
                        __html: item.rationale
                      }} /> : <textarea className={`rank-rationale-textarea responsive-textarea ${errorMessage ? "border-danger" : ""}`} placeholder={positionChanged ? t("Required: Why did you rank this project here?") : t("Why did you rank this project here?")} value={item.rationale || ""} onChange={e => handleRationaleTextareaChange(index, e.target.value)} onBlur={() => handleRationaleBlur(index)} readOnly={isArchived} style={isArchived ? {
                        backgroundColor: '#f8f9fa',
                        cursor: 'not-allowed'
                      } : {}} />}
                                {errorMessage && <small className="text-danger d-block mt-1">
                                    ⚠️ {errorMessage}
                                  </small>}
                              </div>
                            </Accordion.Collapse>
  ...
  return <div className="rank-panel-container responsive-panel compact-mode">
      ...
      <Row className="rank-panel-header responsive-header align-items-center">
        ...
        <Col xs={12} md={4} className="rank-header-buttons responsive-btn-group d-flex justify-content-md-end justify-content-start mt-md-0 mt-3">
          {step === 1 && isAdmin && <Button variant="primary" className="responsive-btn w-100-mobile" onClick={handleNextToRanking} disabled={isGeneratingAI || isArchived || !hasSelectionChanged}>
              {isGeneratingAI ? t("Fetching AI Rankings...") : t("Next: Rank Bets")}
            </Button>}
          {step === 2 && isAdmin && !initialAllRanked && <Button variant="outline-secondary" className="responsive-btn w-100-mobile" onClick={() => setStep(1)}>
              ← {t("Back to Selection")}
            </Button>}
          {step === 2 && (isAdmin || userHasRankingAccess || userHasRerankAccess) && projectList.length > 0 && <Button variant="primary" className="btn-save-rank responsive-btn w-100-mobile" onClick={handleSaveAndClose} disabled={isSaving || isSaved && !hasRankingsChanged() && !userHasRerankAccess && !userHasRankingAccess || isArchived}>
      ...
      {}
      <Modal show={showRationaleModal} onHide={handleRationaleCancel} backdrop="static" keyboard={false} centered>
        <Modal.Header>
          <Modal.Title>
            <div className="d-flex align-items-center gap-2">
        ...
        <Modal.Body>
          <p className="mb-3">
            {t("moving_project_position", {
            project: movedProjectName
          })}
          </p>
          ...
          <Form.Group>
            <Form.Control as="textarea" rows={4} placeholder={t("project_rationale_placeholder")} value={tempRationale} onChange={e => setTempRationale(e.target.value)} autoFocus />
          </Form.Group>
          {pendingDragResult && <div className="mt-3">
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={handleRationaleSubmit} disabled={!tempRationale || tempRationale.trim() === ""}>
            {t("confirm_ranking")}
          </Button>
        </Modal.Footer>
      </Modal>

      <ConfirmationModal show={showConfirmModal} onHide={() => setShowConfirmModal(false)} onConfirm={handleSaveRankings} title={t("Save Rankings?")} message={t("Are you sure you want to save these rankings? This will update the project order for all team members.")} confirmVariant="primary" confirmText={t("Save")} />
    </div>;
};
