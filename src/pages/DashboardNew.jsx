...
const DashboardNew = () => {
  ...
  // Active tab state for toggle navigation
  const [activeBusinessTab, setActiveBusinessTab] = useState("my-businesses");
  ...
  const DeleteButtonAlternatives = ({ business, viewType, canDelete = true }) => {
    ...
    return (
      <div className="business-card-item">
        ...
        >
          <div className="business-card-progress">
            <CircularProgressbar
              value={progress}
          ...
          <h6 className="business-card-name">{business.business_name}</h6>
        ...
        {canDelete && !isViewer && (
          <button
            className="business-card-delete"
            onClick={(e) => {
              e.stopPropagation();
  ...
  const BusinessList = ({ businesses, viewType, canDelete = true }) => (
    <div className={`business-list-grid ${viewType}`}>
      {isLoadingBusinesses && (
        <div className="d-flex justify-content-center align-items-center py-5" style={{ gridColumn: '1 / -1' }}>
          <Spinner animation="border" role="status" variant="primary" />
          <span className="ms-2 text-muted">{t('loading_businesses')}</span>
        </div>
      )}
      {!isLoadingBusinesses && businesses.length === 0 && (
        <div className="text-center text-muted py-5" style={{ gridColumn: '1 / -1' }}>
  ...
  // Get current businesses based on active tab
  const getCurrentBusinesses = () => {
    switch (activeBusinessTab) {
      case "my-businesses":
        return myBusinesses;
      case "project-phase":
        return projectPhaseBusinesses;
      case "collaborating":
        return collaboratingBusinesses;
      default:
        return myBusinesses;
    }
  };
    ...
    <div className="dashboard-layout">
      ...
      <div className="dashboard-header">
        <div className="dashboard-header-left">
          <img src="/traxxia-logo.png" alt="Traxxia" className="dashboard-logo" />        </div>
                ...
                <Card.Body className="p-0 h-100">
                  ...
                  <div className="dashboard-hero">
                    <div className="hero-content">
                      <h1 className="hero-title">{t('welcome_dashboard')}</h1>
                      {!isViewer && (
                        <p className="hero-subtitle">{t('create_business_plans')}</p>
                      )}
                      {!isViewer && (
                  ...
                  {/* Toggle Navigation */}
                  <div className="business-toggle-nav">
                    <button
                      className={`toggle-btn-dashboard ${activeBusinessTab === "my-businesses" ? "active" : ""}`}
                      onClick={() => setActiveBusinessTab("my-businesses")}
                    >
                      {t("my_businesses")}
                      <span className="toggle-count">{myBusinesses.length}</span>
                    </button>
                    <button
                      className={`toggle-btn-dashboard ${activeBusinessTab === "project-phase" ? "active" : ""}`}
                      onClick={() => setActiveBusinessTab("project-phase")}
                    >
                      {t("Project Phase")}
                      <span className="toggle-count">{projectPhaseBusinesses.length}</span>
                    </button>
                    <button
                      className={`toggle-btn-dashboard ${activeBusinessTab === "collaborating" ? "active" : ""}`}
                      onClick={() => setActiveBusinessTab("collaborating")}
                    >
                      Collaborating Businesses
                      <span className="toggle-count">{collaboratingBusinesses.length}</span>
                    </button>
                  </div>

                  {/* Business Cards Grid */}
                  <div className="businesses-grid-section">
                    <div className="business-grid-container">
                      <BusinessList
                        businesses={getCurrentBusinesses()}
                        viewType="grid"
                        canDelete={activeBusinessTab === "my-businesses"}
                      />
                    </div>
                  </div>
                </Card.Body>
