...
import "../styles/businesspage-sidebar.css";
...
const BusinessSetupPageNew = () => {
  const location = useLocation();
  const { selectedBusiness: contextBusiness } = useBusiness();

  // Priority: 1. Context, 2. Location State
  const business = contextBusiness || location.state?.business;
  const selectedBusinessId = business?._id;
  const selectedBusinessName = business?.business_name;
  const companyAdminIds = business?.company_admin_id || [];
  ...
  const handleAnalysisTabClick = () => {
    ...
    if (!isMobile && !isAnalysisExpanded) {
    ...
    <div className="business-setup-container">
      ...
      <div className="business-page-header">
        ...
        <div className="business-header-right">
          <MenuBar selectedBusinessName={selectedBusinessName} />
        </div>
      </div>
      ...
      {questionsLoaded && (
        <div className="business-sidebar">
          <nav className="business-sidebar-nav">
            <button
              className={`business-sidebar-item ${activeTab === "chat" ? "active" : ""}`}
              onClick={handleQuestionsTabClick}
            >
              <span className="sidebar-icon">💬</span>
              <span className="sidebar-label">{t("questions")}</span>
            </button>
            <button
              className={`business-sidebar-item ${activeTab === "brief" ? "active" : ""}`}
              onClick={handleBriefTabClick}
            >
              <span className="sidebar-icon">📄</span>
              <span className="sidebar-label">{t("brief")}</span>
            </button>
            {unlockedFeatures.analysis && (
              <button
                className={`business-sidebar-item ${activeTab === "analysis" ? "active" : ""}`}
                onClick={handleAnalysisTabClick}
              >
                <span className="sidebar-icon">📊</span>
                <span className="sidebar-label">{t("analysis")}</span>
              </button>
            )}
            {unlockedFeatures.analysis && (
              <button
                className={`business-sidebar-item ${activeTab === "strategic" ? "active" : ""}`}
                onClick={handleStrategicTabClick}
              >
                <span className="sidebar-icon">🎯</span>
                <span className="sidebar-label">{t("strategic")}</span>
              </button>
            )}
            {showProjectsTab && (
              <button
                className={`business-sidebar-item ${activeTab === "projects" ? "active" : ""}`}
                onClick={() => setActiveTab("projects")}
              >
                <span className="sidebar-icon">📁</span>
                <span className="sidebar-label">{t("Projects")}</span>
              </button>
            )}
          </nav>
        </div>
      ...
      <div className={`main-container ${isAnalysisExpanded && !isMobile ? "analysis-expanded" : ""}`}>
        <div className={`chat-section ${activeTab !== "chat" ? "hidden" : ""}`}>
        ...
        {questionsLoaded && (
          <div className={`info-panel ${isMobile ? (activeTab === "brief" || activeTab === "analysis" || activeTab === "strategic" || activeTab === "projects" ? "active" : "") : ""}`}>
            {!isMobile && (
...
export default BusinessSetupPageNew;
