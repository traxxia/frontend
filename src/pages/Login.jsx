  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (errorData?.error === 'incorrect_email') {
        setErrors({
          email: t("incorrect_email") !== "incorrect_email" ? t("incorrect_email") : errorData.message || "Incorrect email address"
        });
      } else if (errorData?.error === 'incorrect_password') {
        setErrors({
          password: t("incorrect_password") !== "incorrect_password" ? t("incorrect_password") : errorData.message || "Incorrect password"
        });
      } else {
      ...
      <div className="login-right-section">
        <div className="theme-icon-toggle">
          <button onClick={toggleTheme} className="theme-toggle-button" disabled={isLoading}>
            <FontAwesomeIcon icon={theme === "dark" ? faSun : faMoon} className="login--s1" />
          </button>
        </div>
          ...
          <form onSubmit={handleSubmit} noValidate>
            <div className="form-group">
              <div className="input-container">
                <input type="email" className={errors.email ? "error" : ""} value={email} onChange={e => {
                setEmail(e.target.value);
                if (errors.email) setErrors(prev => ({
                  ...prev,
                  email: ""
                }));
              }} placeholder={t("email_address")} disabled={isLoading} />
              </div>
            ...
            <div className="form-group">
              <div className="input-container">
                <input type={showPassword ? "text" : "password"} className={errors.password ? "error" : ""} value={password} onChange={e => {
                setPassword(e.target.value);
                if (errors.password) setErrors(prev => ({
                  ...prev,
                  password: ""
                }));
              }} placeholder={t("password")} disabled={isLoading} />
                <button type="button" className="toggle-password" onClick={togglePasswordVisibility} disabled={isLoading} aria-label={showPassword ? t("hide_password") : t("show_password")}>
                  <FontAwesomeIcon icon={showPassword ? faEye : faEyeSlash} className="eye-icon login--s2" />
                </button>
              </div>
