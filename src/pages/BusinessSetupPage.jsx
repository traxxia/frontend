...
const BusinessSetupContent = () => {
  ...
  return <div className={`business-setup-container ${isArchived ? 'is-archived' : ''}`}>
      <MenuBar />

      {(currentBusiness?.access_mode === 'archived' || currentBusiness?.access_mode === 'hidden') && <div className="alert alert-warning mb-0 border-0 rounded-0 text-center py-2 d-flex align-items-center justify-content-center shadow-sm business-setup-page--s1">
      ...
      <PhaseUnlockToast phase={phaseManager.unlockedPhase} show={phaseManager.showUnlockToast} onClose={() => phaseManager.setShowUnlockToast(false)} autoCloseMs={2500} />
      ...
      <div className={`main-container ${isAnalysisExpanded && !isMobile ? "analysis-expanded" : ""}`}>
        <div className={`info-panel ${isMobile ? ['advanced', 'insights', 'strategic', 'bets', 'ranking', 'priorities', 'aha', 'executive'].includes(activeTab) ? "active" : "" : ""} ${isAnalysisExpanded && !isMobile ? "expanded" : ""}`}>
          {!isMobile && isAnalysisExpanded && <div className="desktop-expanded-analysis">
              ...
              <div className="desktop-tabs">
                <div className="desktop-tabs-controls">
                  <div className="nav-group-minimal">
                    {hasPmfAccess && <button className={`desktop-tab ${activeTab === "executive" ? "active" : ""}`} onClick={handleExecutiveTabClick}>
                        <LayoutDashboard size={16} />
                        <span>{t("Executive Summary")}</span>
                      </button>}
                    {(hasInsightAccess || hasStrategicAccess) && <button className={`desktop-tab ${activeTab === "advanced" ? "active" : ""}`} onClick={handleBriefTabClick}>
                        <HelpCircle size={16} />
                        <span>{t("Answers/Brief")}</span>
                      </button>}
                    {hasInsightAccess && <button className={`desktop-tab ${activeTab === "insights" ? "active" : ""}`} onClick={handleAnalysisTabClick}>
                        <TrendingUp size={16} />
                        <span>{t("Insights")}</span>
                      </button>}
                    {hasStrategicAccess && <button className={`desktop-tab ${activeTab === "strategic" ? "active" : ""}`} onClick={handleStrategicTabClick}>
                        <Target size={16} />
                        <span>{t("STRATEGIC_LABEL") || "S.T.R.A.T.E.G.I.C."}</span>
                      </button>}
                    {hasPmfAccess && <button className={`desktop-tab ${activeTab === "priorities" ? "active" : ""}`} onClick={handlePrioritiesTabClick}>
                        <ListTodo size={16} />
          ...
          {isMobile && <div className="info-panel-content">
              <TabRenderer />
            </div>}
        </div>
      </div>

      <UpgradeModal show={isModalOpen('upgrade')} onHide={() => closeModal('upgrade')} onUpgradeSuccess={() => window.location.reload()} />

      {hasPmfAccess && <PMFOnboardingModal show={isModalOpen('pmfOnboarding')} onHide={() => closeModal('pmfOnboarding')} businessId={selectedBusinessId} onToastMessage={showToastMessage} onSubmit={() => {
      closeModal('pmfOnboarding');
      setActiveTab("executive");
      setPmfRefreshTrigger(prev => prev + 1);
    }} />}

      <ConfirmationModal show={showConfirmModal} onHide={() => setShowConfirmModal(false)} onConfirm={confirmConfig.onConfirm} title={confirmConfig.title} message={confirmConfig.message} confirmVariant="primary" />

      <PlanLimitModal show={isModalOpen('noFeatureAccess')} onHide={() => closeModal('noFeatureAccess')} title={t('no_access_modal_title')} message={accessModalMessage} subMessage={accessModalSubMessage} isAdmin={['super_admin', 'company_admin', 'org_admin'].includes(userRole?.toLowerCase())} />
    </div>;
};
const BusinessSetupPage = () => {
  return <BusinessSetupProvider>
