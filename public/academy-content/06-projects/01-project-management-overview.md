# Project Management Overview

Transform strategic insights into action with Traxxia's integrated project management. Create, prioritize, and track strategic initiatives directly within your business analysis workflow.

---

## What is Project Management in Traxxia?

Project Management connects strategy to execution by:
- **Creating Strategic Projects**: Turn SWOT insights and strategic analyses into concrete initiatives
- **Prioritizing Initiatives**: Rank projects using custom ranking models
- **Tracking Progress**: Monitor project status and milestones
- **Team Collaboration**: Assign project owners and collaborators

---

## When Projects Unlock

### Business Lifecycle:

**Phase 1: Analysis (Green Card)**
- Complete questionnaire
- Generate strategic analyses  
- Upload financial data
- **No projects yet** - focus on understanding the business

**Phase 2: Projects (Blue Card)**
- Create your first project
- Business card automatically turns blue
- Focus shifts from analysis to execution

> [!IMPORTANT]
> Creating your first project transitions the business from "Analysis" phase to "Projects" phase. This is a one-way transition signaling you're moving from planning to doing.

---

## Creating a Project

### Access Projects Tab:
1. Open your business
2. Click "Projects" tab
3. Click "Create New Project" button

### Project Information:

**Required Fields**:
- **Project Name**: Clear, specific title
- **Description**: What the project accomplishes
- **Owner**: Who's responsible for execution

**Optional Fields**:
- **Start Date**: When work begins
- **End Date**: Target completion
- **Budget**: Estimated cost
- **Priority**: High/Medium/Low
- **Status**: Not Started/In Progress/Completed

![Create Project form](/academy-screenshots/projects/project-creation-form.png)

---

## Project Examples

### From SWOT Analysis:

**Strength-Based Project**:
- SWOT Finding: "Strong AI expertise"
- **Project**: "Launch AI Consulting Service"
- **Description**: "Develop and market AI strategy consulting targeting mid-market companies"

**Opportunity-Based Project**:
- SWOT Finding: "Competitor X exited mid-market"
- **Project**: "Mid-Market Expansion Campaign"
- **Description**: "Sales and marketing push to capture former Competitor X customers"

**Weakness-Mitigation Project**:
- SWOT Finding: "High customer churn (15%)"
- **Project**: "Customer Success Program"
- **Description**: "Implement proactive onboarding and support to reduce churn to <8%"

**Threat-Defense Project**:
- SWOT Finding: "New competitor with $50M funding"
- **Project**: "Product Feature Parity Initiative"
- **Description**: "Develop 5 key features to match competitor capabilities within 6 months"

---

## Project Ranking & Prioritization

### Why Rank Projects?

With limited resources, you can't do everything. Ranking helps:
- Identify highest-impact initiatives
- Allocate resources strategically
- Sequence projects logically
- Communicate priorities to team

### Traxxia's Ranking System:

**Ranking Models**:
Traxxia provides multiple ranking frameworks:
1. **Effort vs. Impact**: Classic 2x2 matrix
2. **Strategic Fit**: Alignment with business goals
3. **Risk vs. Reward**: Evaluate risk tolerance
4. **ROI Ranking**: Financial return focus
5. **Urgency vs. Importance**: Eisenhower Matrix

### How to Rank:

1. Create multiple projects
2. Click "Rank Projects" button
3. Select ranking model
4. Score each project on criteria
5. View prioritized list

![Project ranking interface](/academy-screenshots/projects/project-ranking-interface.png)

---

## Effort vs. Impact Matrix

### Most Common Ranking Model:

**Two Dimensions**:
- **Effort**: Time, resources, complexity required (Low to High)
- **Impact**: Expected business value (Low to High)

**Four Quadrants**:

| Impact | Effort: Low | Effort: High |
|--------|-------------|--------------|
| **High** | 🌟 **Quick Wins** - Do first! | 💎 **Major Projects** - Plan carefully |
| **Low** | 🍎 **Fill-Ins** - Do when available | 💸 **Money Pits** - Avoid or reconsider |

**Strategic Approach**:
1. Start with Quick Wins (high impact, low effort)
2. Plan Major Projects (high impact, high effort)
3. Sprinkle in Fill-Ins when bandwidth allows
4. Avoid or eliminate Money Pits

---

## Project Status Tracking

### Status Options:

**Not Started** 🔵
- Project defined but work hasn't begun
- Resources not yet allocated

**In Progress** 🟡
- Actively working on project
- Team members assigned
- Milestones being tracked

**Completed** ✅
- Project finished
- Deliverables achieved
- Outcomes documented

**On Hold** ⏸️
- Paused temporarily
- Awaiting resources, decisions, or conditions

**Cancelled** ❌
- No longer pursuing
- Resources reallocated

---

## Projects Dashboard View

### What You See:

**Project Card**:
- Project name and description
- Owner
- Status badge
- Priority indicator
- Dates (if set)
- Quick actions (edit, delete, view)

**Sortable and Filterable**:
- Filter by status
- Filter by owner
- Filter by priority
- Sort by name, date, or rank

![Projects dashboard view](/academy-screenshots/projects/ai-assisted-ranking.png)

---

## Team Collaboration on Projects

### Project Roles:

**Project Owner**:
- Primary person responsible
- Accountable for delivery
- Updates status and progress

**Collaborators**:
- All business collaborators can view projects
- Can comment and contribute
- Cannot edit unless given permissions

### Assigning Ownership:
1. Select project
2. Choose owner from dropdown (business collaborators)
3. Save

---

## Project Best Practices

### 1. SMART Projects

Make projects:
- **Specific**: "Launch AI consulting" not "Improve business"
- **Measurable**: Include success metrics  
- **Achievable**: Realistic scope
- **Relevant**: Tied to strategic goals
- **Time-bound**: Set deadlines

### 2. Strategic Alignment

Every project should connect to:
- SWOT insight
- Strategic framework finding
- Clear business objective
- Measurable outcome

**Bad Project**: "General marketing improvements"  
**Good Project**: "Launch content marketing program to address brand awareness weakness, targeting 50 new leads/month"

### 3. Right-Size Projects

**Too Big**: "Complete digital transformation"  
**Too Small**: "Update website footer"

**Just Right**: "Implement CRM system for sales team (Q2 2024)"

### 4. Limit WIP (Work in Progress)

Don't start too many projects simultaneously:
- **Small Team (<10)**: 2-3 active projects max
- **Medium Team (10-50)**: 5-8 active projects
- **Large Team (50+)**: 10-15 active projects

Focus beats diffusion.

---

## Linking Projects to Analyses

### How Projects Connect:

**Strategic Analysis**:
- SWOT generates insights
- Insights suggest projects
- Projects address strengths, weaknesses, opportunities, threats

**Financial Analysis**:
- Financial metrics reveal trends
- Projects improve key metrics
- Track project ROI

**Example Flow**:
1. SWOT identifies weakness: "Manual processes slow customer onboarding"
2. Create project: "Implement Automated Onboarding System"
3. Financial analysis shows: Customer acquisition cost = $500
4. Project goal: Reduce CAC to $300 through automation

---

## Exporting Project Lists

### Export Options:
- **PDF**: Full project portfolio report
- **Excel**: Spreadsheet with all project data
- **Print**: Printable project summary

### Use Cases:
- Board presentations
- Team meetings
- Strategic planning sessions
- Quarterly reviews

---

## Role-Based Access

### Who Can Do What:

| Role | Create Projects | View Projects | Edit Projects | Delete Projects |
|------|----------------|---------------|---------------|-----------------|
| **Super Admin** | ✅ | ✅ | ✅ | ✅ |
| **Company Admin** | ✅ | ✅ | ✅ | ✅ |
| **Collaborator** | ✅ | ✅ | ✅ Owner only | ✅ Owner only |
| **Viewer** | ❌ | ✅ | ❌ | ❌ |

---

## Common Questions

**Q: How many projects should I create?**  
A: Quality over quantity. Start with 3-5 high-impact projects. You can always add more.

**Q: Can I delete a project?**  
A: Yes. Click the project and select "Delete." This is permanent and cannot be undone.

**Q: What happens when I complete a project?**  
A: Mark it "Completed" to track it as finished. It remains visible for reference but can be archived.

**Q: Can projects span multiple businesses?**  
A: No. Projects belong to one business. Create separate projects in each business if needed.

**Q: Does Traxxia have Gantt charts or detailed task management?**  
A: Not currently. Traxxia focuses on strategic project identification and prioritization. Use dedicated project management tools (Asana, Jira, etc.) for detailed execution tracking.

---

## Phase Transition: Analysis → Projects

### What Changes:

**Visual**:
- Dashboard card color: Green → Blue
- "Projects" tab becomes primary focus

**Mindset Shift**:
- From "Understanding the business"
- To "Executing strategy"

**Workflow**:
- Update questionnaire as business evolves
- Regenerate strategic analyses periodically
- Create new projects from updated insights
- Track project progress

> [!TIP]
> You can still update the questionnaire and regenerate analyses even after creating projects. Strategic planning is ongoing, not one-time.

---

## Next Steps

After understanding project management:

1. **[Learn project best practices](./02-creating-strategic-projects.md)** - Detailed project creation guide
2. **[Review SWOT Analysis](../04-strategic-analysis/02-using-swot-analysis.md)** - Source of project ideas
3. **[Invite collaborators](../07-collaboration/02-inviting-collaborators.md)** - Build your project team

---

**Related Articles:**
- [Creating Strategic Projects](./02-creating-strategic-projects.md)
- [Using SWOT Analysis](../04-strategic-analysis/02-using-swot-analysis.md)
- [What is a Business Profile?](../02-businesses/01-what-is-a-business-profile.md)
- [Inviting Collaborators](../07-collaboration/02-inviting-collaborators.md)
